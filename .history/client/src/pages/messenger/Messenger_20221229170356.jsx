...
export default function Messenger() {
  ...
  useEffect(() => {
    ...
    socket?.current?.on("getUsers", (users) => {
      const test = currentUser?.followings.filter((item) =>
        users?.some((u) => u?.userId === item)
      );
      console.log(
        "🚀 ~ file: Messenger.jsx:50 ~ socket?.current?.on ~ test",
        test
      );
    });
    socket?.current?.on("getMessage", (data) => {
      const newMessengerUpdate = {
        text: data?.text,
        conversationId: currentChat?._id,
        sender: data?.senderId,
      };
      setMessages([...messages, newMessengerUpdate]);
    });
  }, [currentChat?._id, currentUser?._id, messages, socket]);
