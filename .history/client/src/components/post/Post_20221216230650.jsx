...
export default function Post({ post }) {
  ...
  const { user: currentUser, setPosts, posts } = useContext(AuthContext);
  ...
  const handleDeletePost = async (postId) => {
    ...
    // });
    const postData =
      posts && posts.length > 0 && posts.filter((item) => item._id !== postId);
    console.log(
      "🚀 ~ file: Post.jsx:82 ~ handleDeletePost ~ postData",
      postData
    );
  };
